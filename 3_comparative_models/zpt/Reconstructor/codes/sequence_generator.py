...
class ReCostorSequenceGenerator(nn.Module):
    def __init__(
        self,
        models,
        tgt_dict,
...
class EnsembleModel(nn.Module):
    ...
    def forward_decoder(
        ...
        for i, model in enumerate(self.models):
            ...
            if self.has_incremental_states():
                decoder_out = model.recostor.forward(
                    tokens,
                    encoder_out=encoder_out,
                    incremental_state=incremental_states[i],
                )
            else:
                decoder_out = model.recostor.forward(tokens, encoder_out=encoder_out)
            ...
            probs = model.get_zp_normalized_probs(
    ...
    def reorder_incremental_state(
        ...
        if not self.has_incremental_states():
            return
        for i, model in enumerate(self.models):
            model.recostor.reorder_incremental_state_scripting(
