...
@dataclass
class TrainReCostorConfig(FairseqDataclass):
    ...
    # new option for doc-level
    fix_parameters: bool = field(
        default=False, metadata={"help": "freeze the parameters of sent-level mt or not"}
    )
    recostor: bool = field(
        default=False,metadata={"help":"start turning recostor parameters else MT tuning"}
    )
    pretrained_checkpoint: Optional[str] = field(
        default='',metadata={"help":"pretrain model to use for initializing transformer encoder and/or decoder"}
    )

@register_task("recostor",dataclass=TrainReCostorConfig)
class REcostorTask(LegacyFairseqTask):
    ...
    cfg: TrainReCostorConfig
    def __init__(self, cfg:TrainReCostorConfig, src_dict, tgt_dict):
        super().__init__(cfg)
    ...
    def setup_task(cls, cfg, **kwcfg):
        ...
        """
        cfg.left_pad_source = utils.eval_bool(cfg.left_pad_source)
        cfg.left_pad_target = utils.eval_bool(cfg.left_pad_target)
    ...
    def build_model(self, cfg):
        ...
        if getattr(cfg, "eval_bleu", False):
            assert getattr(cfg, "eval_bleu_detok", None) is not None, (
                "--eval-bleu-detok is required if using --eval-bleu; "
                "try --eval-bleu-detok=moses (or --eval-bleu-detok=space "
                "to disable detokenization, e.g., when using sentencepiece)"
            )
            detok_args = json.loads(getattr(cfg, "eval_bleu_detok_args", "{}") or "{}")
            self.tokenizer = encoders.build_tokenizer(
                Namespace(
                    tokenizer=getattr(cfg, "eval_bleu_detok", None), **detok_args
                )
            )

            gen_args = json.loads(getattr(cfg, "eval_bleu_args", "{}") or "{}")

            self.sequence_generator = self.build_generator(
                [model], cfg
            )

        if getattr(cfg, 'pretrained_checkpoint', None) is not None:
            a = torch.load(cfg.pretrained_checkpoint)['model']
            keys = a.keys()
