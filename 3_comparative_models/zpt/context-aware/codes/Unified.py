...
class UnifiedModel(FairseqEncoderDecoderModel):
    """
    ...
    def build_encoder(cls, args, src_dict, embed_tokens):
        return UnifiedEncoder(args, src_dict, embed_tokens)
...
class UnifiedEncoder(FairseqEncoder):
    """
    ...
    def __init__(self, args, dictionary, embed_tokens):
        ...
        self.seg_embed =Embedding(3,embed_tokens.embedding_dim,2)
    ...
    def forward_scriptable(
        ...
        seg_x = self.seg_embed(torch.ones_like(src_tokens))
        x +=seg_x
        ...
        seg_ctx = self.seg_embed(torch.zeros_like(context))
        ctx +=seg_ctx
        ...
        for n,layer in enumerate(self.layers):
            if  n ==0:
                x = torch.cat([ctx,x],dim=0)
                encoder_padding_mask=torch.cat([ctx_mask,encoder_padding_mask],dim=1)
                x = layer(
                    x, encoder_padding_mask=encoder_padding_mask if has_pads else None,
                )
                x = x[ctx_l:]
                encoder_padding_mask = encoder_padding_mask[:,ctx_l:]
            else:
                x = layer(
                        x, encoder_padding_mask=encoder_padding_mask if has_pads else None,
                    )
...
@register_model_architecture("unified", "unified_tiny")
...
@register_model_architecture("unified", "unified")
...
@register_model_architecture("unified", "unified_iwslt_de_en")
def unified_iwslt_de_en(args):
    args.encoder_embed_dim = getattr(args, "encoder_embed_dim", 512)
...
@register_model_architecture("unified", "unified_wmt_en_de")
def unified_wmt_en_de(args):
    base_architecture(args)


# parameters used in the "Attention Is All You Need" paper (Vaswani et al., 2017)
@register_model_architecture("unified", "unified_vaswani_wmt_en_de_big")
def unified_vaswani_wmt_en_de_big(args):
